...
class AzureAppServiceAzureCollector extends MSJsonCollector
{
	...
	protected function BuildUrl($aParameters): string
	{
		if (!array_key_exists(self::URI_PARAM_SUBSCRIPTION, $aParameters)) {
			...
			$sUrl .= '/providers/Microsoft.Web/sites?api-version='.$this->sApiVersion;
	...
	protected function DoLookup($aLookupKey, $sDestField): array
	{
		...
		switch ($sDestField) {
			case 'app_service_plan':
				if (array_key_exists('app_service_plan', $aLookupKey) && ($aLookupKey['app_service_plan'] != '')) {
					$sData = strstr($aLookupKey['app_service_plan'], 'serverfarms');
					if ($sData !== false) {
						$aData = explode('/', $sData);
						$sData = $aData[1];
						$sResult = true;
	...
	protected function ProcessLineBeforeSynchro(&$aLineData, $iLineIndex)
	{
		...
		if (!$this->Lookup($aLineData, array('app_service_plan'), 'app_service_plan', $iLineIndex, true, false)) {
	...
	public function Fetch()
	{
		...
		if ($aData !== false) {
			...
			$aData['status'] = strtolower($this->aJson[$this->aJsonKey[$iJsonIdx]]['properties']['state']);
