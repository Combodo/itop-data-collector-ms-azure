...
class AzurePrivateEndpointAzureCollector extends MSJsonCollector
{
	...
	protected function BuildUrl($aParameters): string
	{
		if (!array_key_exists(self::URI_PARAM_SUBSCRIPTION, $aParameters) || !array_key_exists(self::URI_PARAM_RESOURCEGROUP,
			...
			$sUrl .= '/providers/Microsoft.Network/privateEndpoints?api-version='.$this->sApiVersion;
...
//		}
	...
	protected function DoLookup($aLookupKey, $sDestField): array
	{
		...
		switch ($sDestField) {
			...
			case 'azurenetworkinterfaces_list':
				if (array_key_exists('id', $aLookupKey) && ($aLookupKey['id'] != '')) {
					$sData = strstr($aLookupKey['id'], 'networkInterfaces');
					if ($sData !== false) {
						$aData = explode('/', $sData);
						$sData = $aData[1];
						$sResult = true;
	...
	public function Fetch()
	{
		...
		if ($aData !== false) {
			...
			$sNetworkInterfacesList = '';
			$bFirstNetworkInterface = true;
			$aNetworkInterfaces = $this->aJson[$this->aJsonKey[$iJsonIdx]]['properties']['networkInterfaces'];
			foreach ($aNetworkInterfaces as $aNetworkInterface) {
				list($sResult, $sData) = $this->DoLookup($aNetworkInterface, 'azurenetworkinterfaces_list');
				if ($sResult) {
					if ($bFirstNetworkInterface) {
						$bFirstNetworkInterface = false;
					} else {
						$sNetworkInterfacesList .= '|';
					}
					$sNetworkInterfacesList .= 'azurenetworkinterface_id->name:'.$sData;
				}
			}
			$aData['azurenetworkinterfaces_list'] = $sNetworkInterfacesList;
		}
